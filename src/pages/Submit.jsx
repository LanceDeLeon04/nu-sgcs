...
import { CATEGORIES, FEEDBACK_CATEGORIES, YEAR_LEVELS, FEEDBACK_NOTICE, COMPLAINT_NOTICE } from '../lib/constants.js'
    ...
    <PublicShell wide>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <div className="bg-white rounded-2xl border border-slate-100 card-glow p-6 flex flex-col">
          ...
          <p className="text-sm text-slate-600 mt-4 leading-relaxed">{FEEDBACK_NOTICE}</p>
...
function SubmitForm({ type }) {
  ...
  const submit = async (e) => {
    ...
    if (isComplaint) {
      if (!f.student_id.trim()) return setError('Please enter your student ID.')
      if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(f.email.trim())) return setError('Please enter a valid email address.')
    } else if (!anon && f.email.trim() && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(f.email.trim())) {
      return setError('That email address does not look valid.')
    }
      ...
      <PublicShell>
        <div className="bg-white rounded-2xl border border-slate-100 card-glow p-8 text-center">
          ...
          <div className="mt-5 flex items-start gap-2 text-left bg-nugold-50 border border-nugold-200 rounded-xl px-4 py-3 max-w-md mx-auto">
            ...
            <p className="text-xs text-nugold-800 leading-relaxed">Need an actual resolution and trackable actions? <Link to="/submit/complaint" className="font-bold underline">File a formal complaint</Link> instead.</p>
          </div>
      ...
      {isComplaint ? (
        ...
        <div className="mb-6 flex items-start gap-3 bg-nugold-50 border border-nugold-200 rounded-2xl px-4 py-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-nugold-800/90 mt-1 leading-relaxed">{FEEDBACK_NOTICE}</p>
